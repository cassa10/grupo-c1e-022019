...
public interface MenuRepository extends JpaRepository<Menu,Long> {
    ...
    @Query("SELECT m FROM Menu m WHERE m.provider.city = ?1")
    List<Menu> findAllByProviderCity(String location, Sort criteriaSort);

    @Query("SELECT m FROM Menu m WHERE m.name LIKE %?1% AND (?2 member of m.categories)")
    List<Menu> findAllLikeNameAndCategory(String name,CategoryMenu category,Sort criteriaSort);

    @Query("SELECT m FROM Menu m WHERE ?1 member of m.categories AND m.provider.city = ?2")
    List<Menu> findAllByCategoryAndCity(CategoryMenu category,String city,Sort criteriaSort);

    @Query("SELECT m FROM Menu m WHERE m.name LIKE %?1% AND m.provider.city = ?2")
    List<Menu> findAllLikeNameAndCity(String name,String city,Sort criteriaSort);

    @Query("SELECT m FROM Menu m WHERE m.name LIKE %?1% AND m.provider.city = ?3 AND ?2 member of m.categories")
